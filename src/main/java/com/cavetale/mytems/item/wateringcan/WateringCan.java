...
public final class WateringCan implements Mytem {
    ...
    public WateringCan(final Mytems key) {
        this.key = key;
        textColor = key == Mytems.WATERING_CAN
            ? color(0x4BAF98)
            : color(0xfff704);
        this.displayName = text(Text.toCamelCase(key, " "), textColor);
        this.prototype = new ItemStack(key.material);
        this.radius = key == Mytems.WATERING_CAN ? 0 : 1;
        this.maxWater = key == Mytems.WATERING_CAN ? 16 : 64;
    ...
    public void onPlayerRightClick(PlayerInteractEvent event, Player player, ItemStack item) {
        ...
        if (block.getType() == Material.WATER_CAULDRON) {
            ...
            for (int z = -radius; z <= radius && tag.water > 0; z += 1) {
                for (int x = -radius; x <= radius && tag.water > 0; x += 1) {
